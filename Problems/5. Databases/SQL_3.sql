...
-- or
SELECT maker FROM product JOIN printer ON product.model = printer.model
...
SELECT maker FROM product WHERE model IN (SELECT model FROM pc WHERE speed = (SELECT MAX(speed) FROM pc) AND
ram = (SELECT MIN(ram) FROM pc WHERE speed = (SELECT MAX(speed) FROM pc)))
...
SELECT DISTINCT battle FROM outcomes JOIN ships ON ship = name WHERE ships.class = 'Kongo'
...
WHERE numguns = (SELECT MAX(numguns) FROM classes c WHERE c.bore = classes.bore)