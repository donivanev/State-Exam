...
SELECT ship FROM outcomes WHERE result = 'sunk' GROUP BY ship HAVING COUNT(*) >= 1

-- 2. Битките, в които са участвали поне 3 кораба на една и съща страна

SELECT DISTINCT battle FROM outcomes JOIN ships ON ship = name JOIN classes ON ships.class = classes.class
GROUP BY battle, country HAVING COUNT(*) >= 3

-- 3. Имената на класовете, за които няма кораб, пуснат на вода след 1921 г., но имат пуснат поне един кораб

SELECT c.class FROM classes c JOIN ships s ON c.class = s.class GROUP BY c.class HAVING MAX(s.launched) <= 1921
...
SELECT name, COUNT(CASE WHEN outcomes.result = 'damaged' THEN 1 END) AS damaged_count FROM ships LEFT JOIN outcomes
ON name = ship GROUP BY name

-- 5. (*) Намерете за всеки клас с поне 3 кораба броя на корабите от този клас, които са победили в битка (result = 'ok')

SELECT class, COUNT(ship) AS ok_count FROM ships LEFT JOIN outcomes ON name = ship AND result = 'ok'
WHERE class IN (SELECT class FROM ships GROUP BY class HAVING COUNT(*) >= 3) GROUP BY class;
...
SELECT name, YEAR(date) AS battle_year,
  (SELECT COUNT(*) FROM outcomes WHERE battle = battles.name AND result = 'sunk') AS sunk,
  (SELECT COUNT(*) FROM outcomes WHERE battle = battles.name AND result = 'damaged') AS damaged,
  (SELECT COUNT(*) FROM outcomes WHERE battle = battles.name AND result = 'ok') AS ok
FROM battles;
...
(SELECT name FROM ships JOIN classes ON classes.class = ships.class WHERE numguns < 9) GROUP BY battle HAVING COUNT(*) >= 3
AND SUM(CASE WHEN result = 'ok' THEN 1 ELSE 0 END) >= 2

-- Или

SELECT battle FROM outcomes JOIN ships ON outcomes.ship = ships.name JOIN classes ON ships.class = classes.class
WHERE numguns < 9 GROUP BY battle HAVING COUNT(*) >= 3 AND SUM(CASE WHEN result = 'ok' THEN 1 ELSE 0 END) >= 2;